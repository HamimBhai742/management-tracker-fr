...
export default function SignInForm() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      setIsLoading(false);
     document.cookie = `accessToken=${data.data.accessToken}; path=/; SameSite=lax ; max-age=${data.data.accessTokenExpiresIn}; httponly; secure=false`;
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex items-center justify-between animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
        <label className="flex items-center gap-2 cursor-pointer group">
          <input
            ...
            onChange={handleChange}
            className="w-4 h-4 rounded border-zinc-300 dark:border-zinc-700 text-blue-600 focus:ring-blue-500 focus:ring-2 transition-all"
          />
      ...
      <div className="animate-fade-in-up" style={{ animationDelay: '0.5s' }}>
        ...
        <div className="grid grid-cols-2 gap-4 mt-6">
          <button
            type="button"
            className="flex items-center justify-center gap-2 px-4 py-3 border border-zinc-300 dark:border-zinc-700 rounded-xl hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-all duration-200"
          >
